...
{
    return QCoreApplication::translate("pigeoncoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in pigeoncoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in pigeoncoin.qrc)
...
/** Class encapsulating Pigeoncoin Core startup and shutdown.
...
/** Main Pigeoncoin application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // pigeoncoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Pigeoncoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(pigeoncoin);
    Q_INIT_RESOURCE(pigeoncoin_locale);
    ...
    /// 6. Determine availability of data directory and parse pigeoncoin.conf
    ...
    // pigeoncoin: links repeatedly have their payment requests routed to this process:
