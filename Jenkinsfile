...
// which are part of the Pigeoncoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="pigeoncoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /pigeoncoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "pigeoncoin-pigeoncoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from pigeoncoin-pigeoncoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'pigeoncoin-pigeoncoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from pigeoncoin-pigeoncoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /pigeoncoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /pigeoncoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /pigeoncoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /pigeoncoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /pigeoncoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /pigeoncoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /pigeoncoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
